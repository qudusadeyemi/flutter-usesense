...
### Changed
- Bumped the native SDK pins so the runner reports how the subject supplied the document: `ai.usesense:sdk` `4.6.6` → **`4.6.7`** and `UseSenseSDK` `~> 4.6.2` → **`~> 4.6.3`**. No API or runtime change in this plugin.
...
### Changed
- Bumped the native SDK pins so an upload that arrives incomplete is no longer reported as an outage: `ai.usesense:sdk` `4.6.5` → **`4.6.6`** and `UseSenseSDK` `~> 4.6.1` → **`~> 4.6.2`**. No API or runtime change in this plugin.
